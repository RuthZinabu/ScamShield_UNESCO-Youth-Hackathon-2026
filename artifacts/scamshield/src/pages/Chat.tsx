...
export default function Chat() {
  ...
  const handleSend = async () => {
    ...
    try {
        ...
        if (value) {
          ...
          for (const line of lines) {
            if (line.startsWith('data: ')) {
              ...
              if (text !== '[DONE]') {
                // In some SSE setups, JSON is sent or raw text. Assume raw text or handle appropriately.
                // Assuming raw text per chunk based on typical simple SSE setups for AI.
                // Wait, if it's JSON: try parse, else just append.
                try {
      ...
      // Refresh to get final saved message
      ...
      <div className="grid md:grid-cols-[280px_1fr] gap-6 h-full">
        ...
        <Card className="flex flex-col h-full overflow-hidden border-border/60 shadow-lg">
          ...
          <div className="bg-primary/5 p-4 border-b border-primary/10 flex items-center gap-3">
            ...
            <div>
              <h2 className="font-semibold text-foreground">ScamShield Guide</h2>
              <p className="text-xs text-muted-foreground">I help you analyze claims and spot red flags.</p>
            </div>
          </div>
          ...
          >
            {messages.length === 0 && !isStreaming ? (
              <div className="h-full flex flex-col items-center justify-center text-center max-w-md mx-auto p-6">
                ...
                <h3 className="text-xl font-bold font-display mb-2">How can I help you investigate today?</h3>
                <p className="text-muted-foreground text-sm mb-8">
                  Paste a suspicious claim, ask about a common scam tactic, or let's analyze a specific situation together. I won't tell you what's true or false, but I'll help you figure it out.
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full">
                  {["Is this job offer a scam?", "How do deepfakes work?", "Help me verify a news article"].map((q, i) => (
                    <button 
                      key={i} 
                      onClick={() => setInput(q)}
              ...
              <>
                <div className="flex justify-center mb-6">
                  <span className="bg-primary/5 text-primary text-xs px-3 py-1 rounded-full border border-primary/10 flex items-center gap-1.5">
                    <Info className="w-3.5 h-3.5" /> AI Assistant Context Active
                  </span>
                </div>
